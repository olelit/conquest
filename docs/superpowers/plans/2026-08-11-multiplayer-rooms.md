...
## Примечания после реализации

- Task 2: тест «атаковать можно гекс любого соперника» дополнен `{q:4,r:5,ownerId:2}` (атакующему нужен свой соседний гекс) — исправление утверждено ревью.
- Task 2: закомментированная строка `# CONQUEST_WIN_HEX_COUNT: 97` в docker-compose.yml удаляется в Task 6.
- Task 1: `randomTerrain()` берёт сухопутные террейны (`LAND_TERRAINS` без water) — исправление утверждено ревью.
