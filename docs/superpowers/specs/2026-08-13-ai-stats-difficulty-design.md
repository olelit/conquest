...
- `reaction`: в `handleAction` (для человека) и в `applyAiAction` (для ИИ) — если действие относится к гексу, на который ранее была начата атака на владение (любого игрока), зафиксировать `ms = now - attackStartedAt`. Таймеры хранятся в комнате: `private attackStartedAt = new Map<string, number>()` (ключ `${q},${r}`). Заполняется при успешной атаке (в `handleAction` и в `applyAiAction`) по гексу, который имеет владельца; очищается при фиксации реакции, при завершении битвы или через 30 сек (проверка `now - started < 30000`). Реакция фиксируется и для ИИ, и для человека — для сравнительного анализа.
